...
test('create article and delete it - intercept - faker Examples', async ({page, request}) => {
    const randomArticleTitle = faker.lorem.sentence();
    await page.getByText('New Article').click();
    await page.getByRole('textbox', {name: 'Article Title'}).fill(randomArticleTitle);
    await page.getByRole('textbox', {name: 'What\'s this article about?'}).fill('This is a test Description about the Playwright');
    await page.getByRole('textbox', {name: 'Write your article (in markdown)'}).fill('This is a test body');
    await page.getByRole('button', {name: 'Publish Article'}).click();
    ...
    await expect(page.locator('app-article-list h1').first()).toContainText(randomArticleTitle);
